  ...
  * [DISPATCH]() {
    ...
    if (context.argv.AUTO_COMPLETIONS) {
      ...
      this.yargs.getCompletion(this.rawArgv.slice(1), completions => {
       ...
       // handle by self
      yield this.helper.callFn(this.run, [ this.normalizeContext(context) ], this);
    }
  }

  /**
   * normalize context, default behavior is remove `help` / `h` / `version`
   * @param {Object} context - { cwd, argv, rawArgv }
   * @return {Object} return new context
   * @protected
   */
  normalizeContext(context) {
    context.argv.help = undefined;
    context.argv.h = undefined;
    context.argv.version = undefined;
    return context;
  }
